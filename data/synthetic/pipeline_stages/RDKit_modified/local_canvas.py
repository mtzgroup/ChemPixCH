...
class Canvas(CanvasBase):
    ...
    def __init__(self, size, name, imageType='png'):
        if imageType == "pdf":
            ...
            from local_pidPIL import PILCanvas as _Canvas
        else:
    ...
    def addCanvasLine(self, p1, p2, color=(0, 0, 0), color2=None, **kwargs):
        # added randomness to line start and end positions
        if color2 and color2 != color:
            mp = (p1[0] + p2[0]) / 2., (p1[1] + p2[1]) / 2.
            color = convertColor(color)
            self.canvas.drawLine(p1[0]+np.random.uniform(-1.5,1.5), p1[1]+np.random.uniform(-1.5,1.5), 
                                 mp[0]+np.random.uniform(-1.5,1.5), mp[1]+np.random.uniform(-1.5,1.5),
                                 color=color, width=int(kwargs.get('linewidth', 1)), dash=kwargs.get('dash', None))
            color2 = convertColor(color2)
            self.canvas.drawLine(mp[0]+np.random.uniform(-1.5,1.5), mp[1]+np.random.uniform(-1.5,1.5), 
                                 p2[0]+np.random.uniform(-1.5,1.5), p2[1]+np.random.uniform(-1.5,1.5), 
                                 color=color2, width=int(kwargs.get('linewidth', 1)), dash=kwargs.get('dash', None))
        else:
            color = convertColor(color)
            width = kwargs.get('linewidth', 1)
            self.canvas.drawLine(p1[0]+np.random.uniform(-1.5,1.5), p1[1]+np.random.uniform(-1.5,1.5), 
                                 p2[0]+np.random.uniform(-1.5,1.5), p2[1]+np.random.uniform(-1.5,1.5),
                                 color=color, width=int(width)+np.random.randint(-1,1), dash=kwargs.get('dash', None))

    def addCanvasText(self, text, pos, font, color=(0, 0, 0), **kwargs):
        """
        Method which adds atom labels
        """
        text = re.sub(r'\<.+?\>', '', text)
        angle = kwargs.get('angle', 0)
        if font.face not in faceMap:
            # vary font size randomly 
            font = pid.Font(face=font.face, size=font.size + np.random.uniform(-3,3))
        else:
            font = pid.Font(face=faceMap[font.face], size=font.size + np.random.uniform(-3,3))
        txtWidth, txtHeight = self.canvas.stringBox(text, font)
        bw, bh = txtWidth + txtHeight * 0.5, txtHeight * 0.5
        # add random offset
        offset = txtWidth * pos[2] + np.random.uniform(-2,2)
        labelP = pos[0] - txtWidth / 2 + offset, pos[1] + txtHeight / 2
        color = convertColor(color)
        self.canvas.drawString(text, labelP[0], labelP[1], font, color=color, angle=angle)
        return (bw, bh, offset)
