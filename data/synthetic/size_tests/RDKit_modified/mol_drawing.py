...
# hacked up version of https://github.com/rdkit/rdkit/blob/Release_2018_09/rdkit/Chem/Draw/MolDrawing.py
# to allow new fonts
...
class MolDrawing(object):
    ...
    def _drawBond(self, bond, atom, nbr, pos, nbrPos, conf, width=None, color=None, color2=None,
        ...
        if bType == Chem.BondType.SINGLE:
            ...
            if (not crossBond and (bond.IsInRing() or (atom.GetDegree() != 1 and bond.GetOtherAtom(atom).GetDegree() != 1))):
                ...
                fp1, fp2 = self._offsetDblBond(newpos, newnbrPos, bond, atom, nbr, conf, direction=.5, lenFrac=1.0)
                fp3, fp4 = self._offsetDblBond(newpos, newnbrPos, bond, atom, nbr, conf, direction=-.5, lenFrac=1.0)
                if crossBond:
                    fp2, fp4 = fp4, fp2
                addDefaultLine(fp1, fp2)
    ...
    def _drawLabel(self, label, pos, baseOffset, font, color=None, **kwargs):
        color = color or self.drawingOptions.defaultColor
        if self.drawingOptions.atomLabelAngle != 0:
            kwargs['angle'] = numpy.random.uniform(-1*self.drawingOptions.atomLabelAngle, self.drawingOptions.atomLabelAngle)
        x1 = pos[0]
        y1 = pos[1]
    ...
    def AddMol(self, mol, centerIt=True, molTrans=None, drawingTrans=None, highlightAtoms=[],
        ...
        # print "in AddMol, self.drawingOptions =", self.drawingOptions.__dict__
        ...
        for bond in mol.GetBonds():
            ...
            # print idx, nbrIdx, atom.GetSymbol(), nbr.GetSymbol(), bond.GetBondType()
            pos = self.atomPs[mol].get(idx, None)
            nbrPos = self.atomPs[mol].get(nbrIdx, None)
