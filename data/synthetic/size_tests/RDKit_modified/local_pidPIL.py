...
def _pilFontPath(face, size, bold=0):
    ...
    if bold and face != 'symbol':
        facen = "%s-bold-%d.pil" % (face, size)
    else:
        facen = "%s-%d.pil" % (face, size)

    if face in ['ElliotSix', 'Gaegu-Regular', 'GONGN___', 'Note_this', 'GloriaHallelujah', 'journal']:
        facen = face + '.ttf'
    elif face in ['Caroni-Regular', 'Daniel-Regular']:
        facen = face + '.otf'

    path = os.path.join(_fontprefix, facen)
    return path
...
#    import ipdb;ipdb.set_trace()
    if isinstance(face, six.string_types):
        try:
            ...
            pilfont = ImageFont.truetype(_pilFontPath(face, size, font.bold), size)
    else:
        for item in font.face:
            pilfont = None
            try:
...
class PILCanvas(Canvas):
    ...
    def fontAscent(self, font=None):
        ...
        # special handling for some of the hand-drawn fonts
        if font.face in ['ElliotSix', 'Gaegu-Regular', 'GONGN___', 'Note_this']:
            # print "--> ascent special handling for", font.face
            return font.size
    ...
    def fontDescent(self, font=None):
        ...
        # special handling for some of the hand-drawn fonts
        if font.face in ['ElliotSix', 'Gaegu-Regular', 'GONGN___', 'Note_this']:
            # print "--> descent special handling for", font.face
            return font.size / 2
    ...
    def drawString(self, s, x, y, font=None, color=None, angle=0, **kwargs):
        ...
        x = int(x)
        y = int(y)
        # randomise angle that letter is drawn at
