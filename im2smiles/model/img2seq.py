...
class Img2SeqModel(BaseModel):
    ...
    def _run_epoch(self, config, train_set, val_set, epoch, lr_schedule):
        ...
        for i, (img, formula) in enumerate(minibatches(train_set, batch_size)):
            ...
            # update step
            _, loss_eval, summary = self.sess.run([self.train_op, self.loss, self.merged], feed_dict=fd)
            ...
            if i % 10 == 0:
                # Save to tensorboard
                # summary, _ = self.sess.run([self.train_op, self.merged], feed_dict=fd)
                self.file_writer.add_summary(summary, i)
            # self.file_writer.add_summary(loss_eval, i)
